...
// Modified by Syed Tahmid Mahbub
// Cornell Engineering World Health (EWH)
// 02/06/15
...
RF22 rf22(8);        // TX, CSN = digital pin 8, IRQ = digital pin 2
RF22 rf22_2(7, 1);   // RX, CSN = digital pin 7, IRQ = digital pin 3
...
void setup() 
{
  ...
  while (!rf22_2.init())
  {
    Serial.println("RF22_2 init failed!");
    delay(1000);
  }
  Serial.println("RF22 inits succeeded!");
}
...
void test_tx()
{
  ...
  if (!rf22.setFrequency(434.0))
    Serial.println("test_tx: setFrequency failed");
//  else
//    Serial.println("test_tx: setFrequency succeeded");  
  if (!rf22.setModemConfig(RF22::GFSK_Rb2Fd5))
    Serial.println("test_tx: setModemConfig failed");
//  else
//    Serial.println("test_tx: setModemConfig succeeded");
  //rf22.setTxPower(RF22_TXPOW_14DBM);  // weak to limit current
  
//  uint8_t data[] = "hello";
  uint8_t data[25];
  sprintf((char*)data, "<~>n = %u</~>", n++);
...
//  uint8_t data[] = "12345678901234567890123456789012345678901234567890123456789012312345678901234567890123456789012345678901234567890123456789012312345678901234567890123456789012345678901234567890123456789012312345678901234567890123456789012345678901234567890123456789012345";
  ...
  rf22.waitPacketSent();
  Serial.print("test_tx: ");
  Serial.println((char*)data);
//  Serial.println("test_tx: Data sent");
}

// This works with test_tx above
void test_rx()
{
  if (!rf22_2.setFrequency(434.0))
    Serial.println("test_rx: setFrequency failed");
//  else
//    Serial.println("test_rx: setFrequency succeeded");
  if (!rf22_2.setModemConfig(RF22::GFSK_Rb2Fd5))
    Serial.println("test_rx: setModemConfig failed");
//  else
//    Serial.println("test_rx: setModemConfig succeeded");
...
//    delay(1000);
    ...
    if (rf22_2.recv(buf, &len)) // Should fail, no message available
    {
      Serial.println("test_rx: recv 1 succeeded");
      Serial.print("test_rx: got one in user: ");
      Serial.println((char*)buf);
    }
    else
    {
      Serial.println("test_rx: recv 1 failed");
    }
    
//    while (!rf22_2.available())
//    {
//      delay(1000);
//      test_tx();
//    }
    ...
    test_tx();
    rf22_2.waitAvailable();
    
    if (rf22_2.recv(buf, &len))
    {
       Serial.println("test_rx: recv 2 succeeded");
       Serial.print("test_rx: got one in user: ");
       Serial.println((char*)buf);
    }
    else
    {
       Serial.print("test_rx: recv 2 failed");
    }
